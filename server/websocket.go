...
func (p *Plugin) WebSocketMessageHasBeenPosted(connID, userID string, req *model.WebSocketRequest) {
	...
	case clientMessageTypeSDP:
		...
		if !ok {
			// Fallback to "data" field for non-Cloudflare backend
			sdp, ok = req.Data["data"].([]byte)
			if !ok {
		...
		if ok {
			// Cloudflare backend: include tracks in the message data
